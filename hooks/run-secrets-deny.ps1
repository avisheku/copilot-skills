...
try {
    ...
    foreach ($pat in $guard.secretPatterns) {
        if ($text -match $pat) {
            Write-LedgerEntry -Skill 'hook' -Tool 'secrets-deny' -Outcome 'deny' -Root $root | Out-Null
            Write-Output '{"continue":false,"reason":"secret pattern blocked"}'
            exit 0
        }
    }
} catch { }
